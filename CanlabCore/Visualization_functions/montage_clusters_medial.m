...
% ..
    ...
    % get rid of empty clusters; return if all empty
    if isempty(cl), return, end
    for i =1:length(cl),wh(i) = isempty(cl{i}); end; wh = find(wh);
    if length(wh) == length(cl), return, end
    cl(wh) = []; myc(wh) = [];
    ...
    % MODIFIED FOR MEDIAL VIEW
    XYZmm(:,abs(XYZmm(1,:)) > 15) = []; % get rid of non-medial coordinates
    if(isempty(XYZmm))
        disp('No medial activations to display.');
        return
    end
    ...
    XYZ = XYZ([2 3 1],:);  % reorganize to permuted dims
    oimg = permute(oimg,[2 3 1]);
    ...
    mcmh = create_figure('SCNlab_Montage_Medial');
    colormap gray;
    set(mcmh,'Color','w','MenuBar','none');
    ssize = get(0,'screensize');
    set(mcmh,'Position',ssize ./ [0.0303    0.0128    2.0447    1.2577]);
    ...
    index = plot_cluster(mcmh,cl{1},oimg,rc,V,whsl,myc{1},textx,texty,1,size(oimg),colorlimits);
    if ~isempty(XYZpts), ph = plot_points(mcmh,XYZpts,rc,V,whsl,bcolor,myplab); end
    % plot points for single-voxel clusters
    if isempty(colorlimits)
        for i = 1:length(clusters),
            try
                if clusters(i).numVox == 1, plot_points(mcmh,clusters(i).XYZmm,rc,V,whsl,myc{1},myplab); end
            catch
            end
        end
    end
    ...
    if length(cl) > 1
        for i = 2:length(cl)
            index = plot_cluster(mcmh,cl{i},[],rc,V,whsl,myc{i},textx,texty,i,size(oimg),colorlimits);
            if isempty(colorlimits)
                for j = 1:length(cl{i}),
                    try
                        if cl{i}(j).numVox == 1, plot_points(mcmh,cl{i}(j).XYZmm,rc,V,whsl,myc{i},myplab); end
                    catch
                    end
                end
            end
    ...
    if plotovl
        ...
        if ~isempty(XYZmm_both)
            ...
            plot_cluster(mcmh,bcl,[],rc,V,whsl,bcolor,textx,texty,i+1,size(oimg),colorlimits,1);
        end

        if ~isempty(XYZmm_all)
            bcl.XYZmm = XYZmm_all; bcl.M = cl{1}(1).M;
            plot_cluster(mcmh,bcl,[],rc,V,whsl,acolor,textx,texty,i+1,size(oimg),colorlimits,1);
        end
...
function index = plot_cluster(mcmh,clusters,oimg,rc,V,whsl,myc,textx,texty,clind,odims,colorlimits,varargin)
    ...
    for z = whsl
        ...
        if ~isempty(oimg)
            ...
            xmm = voxel2mm([z 1 1]',V.mat);
            text(textx,texty,['x = ' num2str(round(xmm(1)))],'Color','k','FontSize',fsz)
        else
            hold on
        end
        ...
        if doimgpatch   %isempty(colorlimits)
            ...
            for j = 1:size(myxyz,2)
                ...
                if isempty(colorlimits)
                    h2(j) = patch([myxyz(1,j) myxyz(1,j) myxyz(1,j)+xs(1) myxyz(1,j)+xs(1)], ...
    ...
    if length(varargin) == 0
        ...
        %ssize = get(0,'ScreenSize');
        %set(mcmh,'Position',ssize./[0.0370    0.0014    2.7948   15.2500])
    end
...
function ph = plot_points(mcmh,XYZmm,rc,V,whsl,myc,myplab)
